...
const RecruiterDashboard = () => {
    ...
    return (
        <div className="recruiter-dashboard">
            ...
            <div className="welcome-section">
                <div className="welcome-content">
                    <h1>Welcome back, {profile?.company_name}! 👋</h1>
                    <p>Here's what's happening with your job postings today.</p>
                </div>
                <Link to="/post-job" className="btn btn-primary btn-large">
            ...
            <div className="stats-grid">
                ...
                <div className="stat-card">
                    <div className="stat-icon applications"><i className="fas fa-users"></i></div>
                    <div className="stat-details">
                        <h3>{stats?.total_applications || 0}</h3>
                        <p>Total Applications</p>
                    </div>
            ...
            <div className="dashboard-grid">
                ...
                <div className="dashboard-card recent-jobs">
                    <div className="card-header">
                        <h3><i className="fas fa-briefcase"></i> Your Active Jobs</h3>
                        <Link to="/manage-jobs" className="view-all">Manage Jobs</Link>
                    </div>
                    <div className="jobs-list">
                        {recent_jobs?.length > 0 ? (
                            recent_jobs.map((job) => (
                                <div key={job.id} className="job-item">
                                    <div className="job-info">
                                        <h4>{job.title}</h4>
                                        <div className="job-meta">
                                            <span><i className="fas fa-dollar-sign"></i> ${job.pay_per_hour}/hr</span>
                                            <span><i className="fas fa-users"></i> {job.applications_count || 0} applicants</span>
                                            <span className={`status-badge ${job.is_active ? 'active' : 'inactive'}`}>
                                                {job.is_active ? 'Active' : 'Closed'}
                                            </span>
                                        </div>
                                    </div>
                                    <Link to="/recruiter/jobs" className="btn btn-outline btn-small">View Applications</Link>
                                </div>
                            ))
                        ) : (
                            <div className="empty-state">
                ...
                <div className="dashboard-card recent-applications">
                    <div className="card-header">
                        <h3><i className="fas fa-file-alt"></i> Recent Applications</h3>
                        <Link to="/recruiter/applications" className="view-all">View All</Link>
                    </div>
                    <div className="applications-list">
                        {recent_applications?.length > 0 ? (
                            recent_applications.map((app) => (
                                <div key={app.id} className="application-item">
                                    <div className="application-info">
                                        ...
                                        <p className="job-title">Applied for: {app.job_title}</p>
                                        <div className="application-meta">
                                            <span className="applied-date">{new Date(app.applied_at).toLocaleDateString()}</span>
                ...
                <div className="dashboard-card notifications">
                    ...
                    <div className="notifications-list">
                        {notifications?.length > 0 ? (
                            ...
                            <div className="empty-state"><i className="fas fa-bell-slash"></i><p>No notifications</p></div>
                        )}
                    </div>
                </div>
                ...
                <div className="dashboard-card quick-stats">
                    <div className="card-header">
                        <h3><i className="fas fa-chart-line"></i> Hiring Insights</h3>
                    </div>
                    <div className="stats-insights">
                        <div className="insight-item">
                            <div className="insight-label">
                            ...
                            <div className="progress-bar">
                                <div className="progress-fill" style={{ width: `${Math.min((stats?.total_applications / (stats?.total_jobs * 10)) * 100, 100)}%` }}></div>
                            </div>
                        </div>
                        <div className="insight-item">
                            <div className="insight-label">
                                <span>Acceptance Rate</span>
                                <span className="insight-value">
                                    {stats?.total_applications ? ((stats?.accepted_applications / stats?.total_applications) * 100 || 0).toFixed(1) : 0}%
                                </span>
                            </div>
                            <div className="progress-bar">
                                <div className="progress-fill success" style={{ width: `${stats?.total_applications ? (stats?.accepted_applications / stats?.total_applications) * 100 : 0}%` }}></div>
                            </div>
                        </div>
                        <div className="stats-summary">
                            <div className="summary-item">
                            ...
                            <div className="summary-item">
                                <span className="summary-label">Account Created</span>
                                <span className="summary-value">{new Date(profile?.created_at).toLocaleDateString()}</span>
                            </div>
                        </div>
                    </div>
                </div>
